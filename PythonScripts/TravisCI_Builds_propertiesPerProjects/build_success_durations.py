    ...
    def process_csv(self):
        ...
        for index,row in project_csv.iterrows():
            ...
            if problem_mode and row['BuildState'] != 'passed' :
                ...
                try:
                    total_prob_builds_duration+=int(row['BuildDuration'])
                except Exception as e:
                    pass
            if problem_mode and row['BuildState'] == 'passed':
                builds_in_between_list.append(nb_prob_builds_in_between)
                duration_in_between_list.append(total_prob_builds_duration)
                nb_prob_builds_in_between=0
                total_prob_builds_duration=0
                problem_mode=False
        avg_builds_nb=0
        avg_builds_duration=0
        print(builds_in_between_list)
        print(duration_in_between_list)
...
if __name__ == "__main__":
    ...
    csv_res = open('../Project Stats Summary/failure_fix_nb_duration.csv', 'w+')
    csv_res.write('ProjectName,Type,AvgNbBuilds,AvgBuildsDuration')
    csv_res.write('\n')
