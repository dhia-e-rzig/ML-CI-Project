...
class commit_failure_classifier():
    def __init__(self, git_uri,prob_commit,fix_commit,type):
    ...
    def find_commit_fail_type(self):
        ...
        files_commit = [os.path.join('./commits_jobs_prob_for_transition/', f) for f in files if
                        str(self.prob_commit) in str(f)]
...
if __name__ == "__main__":
    ...
    tool.fillna('0')
    # pprint(tool.columns)
    tool=tool.loc[tool['MainProblem']=='Unknown'] #
    pprint(len(tool))
    # pprint(tool['MainProblem'].to_list())
    # exit()
    #[commit_failure_classifier(row['ProjectGitRepo'],row['Failed-ErrorredCommitID'],row['FixCommitID'],'Tool') for index,row in applied.iterrows()]+
    list_of_objects = [commit_failure_classifier(row['ProjectGitRepo'],row['Failed-ErrorredCommitID'],row['FixCommitID'],'Tool') for index,row in tool.iterrows()]
    print(len(list_of_objects))
    pool = mp.Pool(NUM_CORE)
    list_of_results = pool.map(worker, ((obj) for obj in list_of_objects))
    pool.close()
    pool.join()
    csv_res = open('../../CSV Outputs/build_problem_commits_v5_unknown_tool_fix.csv', 'w+')
    csv_res.write(
