...
df_truth=pd.read_csv('../../CSV Inputs/csv_for_eval_error/files_ground_truth.csv')
...
df_v1_regex=pd.read_csv('../../CSV Inputs/csv_for_eval_error/error_classification_v1.csv')
df_v1_regex=df_v1_regex.sort_values(by='file_name')

classes=['True','False']

classification_df_list=[df_v1_regex] #,df_1pass_bigram,df_1pass_trigram,df_2pass,df_3pass

classification_df_name_list=['v1_err_classifier'] #','df_1pass_bigram','df_1pass_trigram','df_2pass','df_3pass'
import numpy as np
...
def test_classifier_for_property(type,classification_df,df_name):
    ...
    # file_name_diff=list(set(classification_df['file_name'].to_list())- set(file_name_list)  )
    # file_name_diff=list(set(file_name_list)-set(classification_df['file_name'].to_list())  )
    # print(list(set([x for x in file_name_list if file_name_list.count(x) > 1])))
...
types =['FailureClassedAsError','ScriptError','BuildDepError','TravisError']
